import {parseInterface, replaceInterface} from "../server/interface-parser";
...
describe("interface-parser", () => {
  describe("#parseInterface", () => {
    ...
    it("should throw an error if a device is not found", () => {
      const device = "eth23";
      let thrown = false;
      try {
        parseInterface(testData, device);
      } catch(error) {
        thrown = true;
      }
      if(!thrown) throw new Error("Failed to throw error");
    });
  });
});